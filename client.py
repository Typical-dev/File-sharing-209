...
def recieveMessage():
    ...
    while True:
        chunk = SERVER.recv(BUFFER_SIZE)
        try:
            if("tiul" in chunk.decode() and "1:" not in chunk.decode()):
                letterList = chunk.decode().split(",")
                listbox.insert(letterList[0]+":"+letterList[1]+":"+letterList[3]+":"+letterList[5])
                print(letterList[0]+":"+letterList[1]+":"+letterList[3]+":"+letterList[5])
            else:
